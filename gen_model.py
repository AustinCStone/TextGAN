...
class GenModel():
    def __init__(self, args):
        ...
        with tf.variable_scope('GEN') as scope:
            ...
            self.latent_to_initial_state = tf.Variable(tf.random_normal([args.latent_size, args.rnn_size], stddev=0.35, dtype=tf.float32), name='latent_to_intial_state')
            self.initial_state = tf.matmul(self.latent_state, self.latent_to_initial_state)
    ...
    def generate(self):
        ...
        with tf.variable_scope('GEN', reuse=self.has_init_seq2seq) as scope:
            ...
            outputs, last_state = seq2seq.rnn_decoder(inputs, [self.initial_state], self.cell, loop_function=loop, scope=scope)
